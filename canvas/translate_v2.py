...
# m01
html = html.replace(
    "description: 'This mental model is based on the idea that any representation of reality",
    "description: '这个思维模型基于这样一个理念：任何对现实的表征"
)

html = html.replace(
    "be it a physical map, a business plan, a scientific theory, or a data model",
    "无论是物理地图、商业计划、科学理论还是数据模型"
)

html = html.replace(
    "is inherently a simplification and not reality itself. These \\\"maps\\\" are useful for navigation and understanding, but they are incomplete and can be misleading if we forget they are abstractions. Reality (\\\"the territory\\\") is infinitely more complex and detailed than any model we can create.'",
    "本质上都是简化，而非现实本身。这些"地图"对导航和理解很有用，但如果我们忘记它们是抽象的，就可能产生误导。现实（"领土"）比我们能创建的任何模型都更加复杂和详细。'"
)
...
# m03
html = html.replace(
    "description: 'First-principles thinking is the act of deconstructing a problem or idea down to its most fundamental, foundational truths",
    "description: '第一性原理思维是将问题或想法解构到最基本、最基础的真理"
)

html = html.replace(
    "the things you know are true beyond any doubt. From there, you reason up from scratch, challenging all assumptions and conventions. It\\'s the opposite of reasoning by analogy (i.e., \\\"we do it this way because that\\'s how it\\'s always been done\\\").'",
    "那些你毫无疑问知道是正确的东西。然后，你从零开始推理，挑战所有假设和惯例。这是类比推理的对立面（即"我们这样做是因为一直都是这样做的"）。'"
)
...
# m09
html = html.replace(
    "description: 'Occam\\'s Razor is a problem-solving principle that states when you are presented with multiple competing explanations for a phenomenon, the simplest one",
    "description: '奥卡姆剃刀是一个解决问题的原则，它指出当你面对一个现象的多种竞争解释时，最简单的那个"
)
...
html = html.replace(
    "is the most likely to be correct. It\\'s a mental razor that \\\"shaves away\\\" unnecessary complexity.'",
    "最可能是正确的。它是一把"剃掉"不必要复杂性的心智剃刀。'"
)
...
html = html.replace(
    "description: 'Evolution is the long-term process of change driven by natural selection. Organisms or systems with traits best suited to their environment",
    "description: '进化是由自然选择驱动的长期变化过程。具有最适合环境特征的生物或系统"
)
...
print("\n已翻译:")
print("- Category字段: 9个类别")
print("- Description字段: 前15个模型")
print("- Keywords字段: 主要关键词")