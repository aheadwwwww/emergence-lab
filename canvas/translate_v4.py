...
# 使用原始字符串和字典进行description翻译
desc_map = {
    "This mental model is based on the idea that any representation of reality": "这个思维模型基于这样一个理念：任何对现实的表征",
    "be it a physical map, a business plan, a scientific theory, or a data model": "无论是物理地图、商业计划、科学理论还是数据模型",
    "is inherently a simplification and not reality itself": "本质上都是简化，而非现实本身",
    "These maps are useful for navigation and understanding": "这些地图对导航和理解很有用",
    "but they are incomplete and can be misleading if we forget they are abstractions": "但如果我们忘记它们是抽象的，就可能产生误导",
    ...
    "First-principles thinking is the act of deconstructing a problem or idea down to its most fundamental, foundational truths": "第一性原理思维是将问题或想法解构到最基本、最基础的真理",
    "the things you know are true beyond any doubt": "那些你毫无疑问知道是正确的东西",
    ...
    "we do it this way because that's how it's always been done": "我们这样做是因为一直都是这样做的",
    ...
    "It's a way of thinking through possibilities, testing hypotheses, and uncovering implications in the laboratory of the mind": "它是一种在心智实验室中思考可能性、检验假设和发现影响的方式",
    "This is the practice of thinking beyond the immediate consequences of a decision and considering the longer-term": "这是一种超越决策的直接后果、考虑更长期效应的实践",
    "and then what effects": "然后呢效应",
    ...
    "How can I achieve success": "我如何取得成功",
    "What could cause me to fail": "什么可能导致我失败",
    ...
    "Never attribute to malice that which is adequately explained by stupidity": "永远不要把可以用愚蠢解释的事情归咎于恶意",
    "or incompetence, ignorance, or neglect": "或无能、无知或疏忽",
    "It suggests that when something goes wrong or someone acts in a way that negatively affects you": "它表明当出了问题或有人以对你产生负面影响的方式行事时",
    "it's far more likely to be the result of a mistake or oversight than a deliberate attempt to cause you harm": "这更可能是错误或疏忽的结果，而不是故意伤害你的企图",
    "when you are presented with multiple competing explanations for a phenomenon": "当你面对一个现象的多种竞争解释时",
    "the simplest one": "最简单的那个",
    "the one with the fewest assumptions": "需要最少假设的那个",
    "is the most likely to be correct": "最可能是正确的",
    "It's a mental razor that shaves away unnecessary complexity": "它是一把剃掉不必要复杂性的心智剃刀",
    "Activation energy is the initial spark of effort required to start a process or reaction": "活化能是启动过程或反应所需的初始努力火花",
    "Just as a chemical reaction needs a certain amount of energy to begin": "正如化学反应需要一定能量才能开始",
    "many tasks, projects, and habits require an upfront investment of energy to overcome inertia and get started": "许多任务、项目和习惯需要前期能量投资来克服惯性并开始",
    ...
    "This concept extends beyond metallurgy to teams, skills, and ideas": "这个概念超越了冶金学，延伸到团队、技能和想法",
    "where a strategic mix of diverse components can create a stronger, more resilient, and more valuable whole": "战略性地混合多样化组件可以创造更强大、更有韧性、更有价值的整体",
    ...
    "In broader terms, a catalyst is a person, event, or technology that precipitates significant change or action": "在更广泛的术语中，催化剂是促成重大变化或行动的人、事件或技术",
    "without necessarily being a major part of the final outcome": "而不一定是最终结果的主要部分",
    "They lower the activation energy required for a change to occur": "它们降低变化发生所需的活化能",
    "making it faster, more likely, or even possible in the first place": "使其更快、更有可能、甚至首先成为可能",
    ...
    "From symbiotic relationships in biology to alliances in human society": "从生物学中的共生关系到人类社会中的联盟",
    "cooperation allows for the pooling of resources, knowledge, and capabilities to achieve outcomes that would be impossible for individuals alone": "合作允许汇集资源、知识和能力，以实现个人无法实现的结果",
    "An ecosystem is a complex network of interconnected and interdependent parts": "生态系统是一个相互关联和相互依存的复杂网络",
    "No single component exists in isolation": "没有一个组件是孤立存在的",
    "the actions of one element can have cascading, often unpredictable, effects on the entire system": "一个元素的行动可以对整个系统产生连锁的、往往是不可预测的影响",
    "This model emphasizes understanding the relationships, feedback loops, and emergent behaviors within a system": "这个模型强调理解系统内的关系、反馈循环和涌现行为",
    "rather than just analyzing its individual components": "而不仅仅是分析其各个组件",
}
...
print(f"翻译完成！源文件: {os.path.getsize(source_path)/1024:.1f}KB, 目标文件: {os.path.getsize(target_path)/1024:.1f}KB")