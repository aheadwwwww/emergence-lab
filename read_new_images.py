...
def extract_text_from_image(image_path):
    ...
    data = {
        "contents": [{
            "parts": [
                {"text": "Extract all text from this image. If it's a chat screenshot, extract each message and sender. Keep original format."},
                {
                    "inline_data": {
...
images = sorted(media_dir.glob('*.jpg'), key=lambda x: x.stat().st_mtime, reverse=True)[:2]
...
for img in images:
    ...
    # 保存
    output_file = pathlib.Path.home() / '.openclaw' / 'workspace' / f'xuheng_{img.stem}.txt'
